...
public class SuperFreezingBasinRecipe extends ProcessingRecipe<SmartInventory> {
    ...
    private static boolean apply(BasinBlockEntity basin, Recipe<?> recipe, boolean test) {
        ...
        BlazeFreezerBlock.FreezingLevel freezingLevel = BlazeFreezerBlock.getFreezingLevelOf(basin.getLevel()
                .getBlockState(basin.getBlockPos()
                        .below(1)));
        CreateIceAge.LOGGER.info("Freezing level: " + freezingLevel);
        if (freezingLevel.equals(BlazeFreezerBlock.FreezingLevel.NONE))
            return false;
//        if (isBasinRecipe && !((BasinRecipe) recipe).getRequiredHeat()
//                .testBlazeBurner(freezingLevel))
//            return false;
...
//            return false;
    ...
    protected SuperFreezingBasinRecipe(IRecipeTypeInfo type, ProcessingRecipeBuilder.ProcessingRecipeParams params) {
        super(type, params);
    }

    public SuperFreezingBasinRecipe(ProcessingRecipeBuilder.ProcessingRecipeParams params) {
