...
public class AllCreativeModeTabs {
    private static final DeferredRegister<CreativeModeTab> REGISTER = DeferredRegister
            .create(Registries.CREATIVE_MODE_TAB, CreateIceAge.MOD_ID);

    public static final DeferredHolder<CreativeModeTab, CreativeModeTab> BASE_CREATIVE_TAB = REGISTER.register("base",
            () -> CreativeModeTab.builder()
                    .title(Component.translatable("itemGroup.create_ice_age.base"))
                    .withTabsBefore(CreativeModeTabs.SPAWN_EGGS)
                    .icon(AllItems.ICE_CAKE::asStack)
    ...
    private static class RegistrateDisplayItemsGenerator implements CreativeModeTab.DisplayItemsGenerator {
        ...
        private List<Item> collectBlocks(Predicate<Item> exclusionPredicate) {
            ...
            for (RegistryEntry<Block, Block> entry : CreateIceAge.registrate().getAll(Registries.BLOCK)) {
...
//                    System.out.println("Excluding item: " + item);
        ...
        private List<Item> collectItems(Predicate<Item> exclusionPredicate) {
            List<Item> items = new ReferenceArrayList<>();
            for (RegistryEntry<Item, Item> entry : CreateIceAge.registrate().getAll(Registries.ITEM)) {
//                if (!CreateRegistrate.isInCreativeTab(entry, tabFilter))
//                    continue;
                Item item = entry.get();
