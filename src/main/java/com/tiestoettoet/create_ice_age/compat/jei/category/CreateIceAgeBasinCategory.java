...
public class CreateIceAgeBasinCategory extends CreateIceAgeRecipeCategory<CreateIceAgeBasinRecipe> {

    private final boolean needsFreezing;

	public CreateIceAgeBasinCategory(Info< CreateIceAgeBasinRecipe > info, boolean needsFreezing) {
        super(info);
        this.needsFreezing = needsFreezing;
    }

    @Override
    public void setRecipe(IRecipeLayoutBuilder builder, CreateIceAgeBasinRecipe recipe, IFocusGroup focuses) {
    ...
    @Override
    public void draw(CreateIceAgeBasinRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics graphics, double mouseX, double mouseY) {
        FreezeCondition requiredFreeze = recipe.getRequiredFreeze();

        boolean noHeat = requiredFreeze == FreezeCondition.NONE;
        ...
        graphics.drawString(Minecraft.getInstance().font, CreateIceAgeLang.translateDirect(requiredFreeze.getTranslationKey()), 9,
                86, requiredFreeze.getColor(), false);
    }
