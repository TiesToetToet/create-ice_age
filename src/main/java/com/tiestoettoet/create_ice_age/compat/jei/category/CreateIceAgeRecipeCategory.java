...
import com.tiestoettoet.create_ice_age.compat.jei.CreateIceAgeJEI;
...
public abstract class CreateIceAgeRecipeCategory<T extends Recipe<?>> implements IRecipeCategory<RecipeHolder<T>> {
    ...
    public CreateIceAgeRecipeCategory(CreateIceAgeRecipeCategory.Info<T> info) {
    ...
    public static IRecipeSlotRichTooltipCallback addStochasticTooltip(ProcessingOutput output) {
        return (view, tooltip) -> {
            ...
            if (chance != 1)
                tooltip.add(CreateIceAgeLang.translateDirect("recipe.processing.chance", chance < 0.01 ? "<1" : (int) (chance * 100))
    ...
    public static IRecipeSlotBuilder addFluidSlot(IRecipeLayoutBuilder builder, int x, int y, SizedFluidIngredient ingredient) {
        ...
        return builder.addSlot(RecipeIngredientRole.INPUT, x, y)
                ...
                .addTooltipCallback(CreateIceAgeRecipeCategory::addPotionTooltip);
    }
    ...
    public interface Factory<T extends Recipe<?>> {
        CreateIceAgeRecipeCategory<T> create(CreateIceAgeRecipeCategory.Info<T> info);
    }
    ...
    public static class Builder<T extends Recipe<? extends RecipeInput>> {
        ...
        public CreateIceAgeRecipeCategory.Builder<T> enableWhen(Supplier<Boolean> predicate) {
            this.config = predicate;
            return this;
        }

        public CreateIceAgeRecipeCategory.Builder<T> enableWhen(ConfigBase.ConfigBool configValue) {
            config = configValue::get;
            return this;
        }

        public CreateIceAgeRecipeCategory.Builder<T> addRecipeListConsumer(Consumer<List<RecipeHolder<T>>> consumer) {
            recipeListConsumers.add(consumer);
            return this;
        }

        public CreateIceAgeRecipeCategory.Builder<T> addRecipes(Supplier<Collection<? extends RecipeHolder<T>>> collection) {
            return addRecipeListConsumer(recipes -> recipes.addAll(collection.get()));
        }

        public CreateIceAgeRecipeCategory.Builder<T> addAllRecipesIf(Predicate<RecipeHolder<T>> pred) {
            return addRecipeListConsumer(recipes -> consumeAllRecipesOfType(recipe -> {
                if (pred.test(recipe)) recipes.add(recipe);
            }));
        }

        public CreateIceAgeRecipeCategory.Builder<T> addAllRecipesIf(Predicate<RecipeHolder<?>> pred, Function<RecipeHolder<?>, RecipeHolder<T>> converter) {
            return addRecipeListConsumer(recipes -> CreateIceAgeJEI.consumeAllRecipes(recipe -> {
                if (pred.test(recipe)) {
        ...
        public CreateIceAgeRecipeCategory.Builder<T> addTypedRecipes(IRecipeTypeInfo recipeTypeEntry) {
            return addTypedRecipes(recipeTypeEntry::getType);
        }

        public <I extends RecipeInput, R extends Recipe<I>> CreateIceAgeRecipeCategory.Builder<T> addTypedRecipes(Supplier<net.minecraft.world.item.crafting.RecipeType<R>> recipeType) {
            return addRecipeListConsumer(recipes -> CreateIceAgeJEI.<T>consumeTypedRecipes(recipe -> {
        ...
        public CreateIceAgeRecipeCategory.Builder<T> addTypedRecipes(Supplier<net.minecraft.world.item.crafting.RecipeType<T>> recipeType, Function<RecipeHolder<?>, RecipeHolder<T>> converter) {
            return addRecipeListConsumer(recipes -> CreateIceAgeJEI.<T>consumeTypedRecipes(recipe -> recipes.add(converter.apply(
                    recipe)), recipeType.get()));
        }

        public CreateIceAgeRecipeCategory.Builder<T> addTypedRecipesIf(Supplier<net.minecraft.world.item.crafting.RecipeType<? extends T>> recipeType, Predicate<RecipeHolder<?>> pred) {
        ...
        public CreateIceAgeRecipeCategory.Builder<T> addTypedRecipesExcluding(
            ...
            return addRecipeListConsumer(recipes -> {
                List<RecipeHolder<?>> excludedRecipes = CreateIceAgeJEI.getTypedRecipes(excluded.get());
                consumeTypedRecipesTyped(recipe -> {
                    for (RecipeHolder<?> excludedRecipe : excludedRecipes) {
                        if (CreateIceAgeJEI.doInputsMatch(recipe.value(), excludedRecipe.value())) {
                            return;
                        }
                    }
                    recipes.add(recipe);
        ...
        public CreateIceAgeRecipeCategory.Builder<T> removeRecipes(Supplier<net.minecraft.world.item.crafting.RecipeType<? extends T>> recipeType) {
            return addRecipeListConsumer(recipes -> {
                List<RecipeHolder<?>> excludedRecipes = CreateIceAgeJEI.getTypedRecipes(recipeType.get());
                recipes.removeIf(recipe -> {
                    for (RecipeHolder<?> excludedRecipe : excludedRecipes) {
                        if (CreateIceAgeJEI.doInputsMatch(recipe.value(), excludedRecipe.value()) &&
                                CreateIceAgeJEI.doOutputsMatch(recipe.value(), excludedRecipe.value())) return true;
                    }
                    return false;
                });
            });
        ...
        public CreateIceAgeRecipeCategory.Builder<T> removeNonAutomation() {
            return addRecipeListConsumer(recipes -> recipes.removeIf(AllRecipeTypes.CAN_BE_AUTOMATED.negate()));
        }

        public CreateIceAgeRecipeCategory.Builder<T> catalystStack(Supplier<ItemStack> supplier) {
            catalysts.add(supplier);
            return this;
        }

        public CreateIceAgeRecipeCategory.Builder<T> catalyst(Supplier<ItemLike> supplier) {
            return catalystStack(() -> new ItemStack(supplier.get().asItem()));
        }

        public CreateIceAgeRecipeCategory.Builder<T> icon(IDrawable icon) {
            this.icon = icon;
            return this;
        }

        public CreateIceAgeRecipeCategory.Builder<T> itemIcon(ItemLike item) {
            icon(new ItemIcon(() -> new ItemStack(item)));
            return this;
        }

        public CreateIceAgeRecipeCategory.Builder<T> doubleItemIcon(ItemLike item1, ItemLike item2) {
            icon(new DoubleItemIcon(() -> new ItemStack(item1), () -> new ItemStack(item2)));
            return this;
        }

        public CreateIceAgeRecipeCategory.Builder<T> background(IDrawable background) {
            this.background = background;
            return this;
        }

        public CreateIceAgeRecipeCategory.Builder<T> emptyBackground(int width, int height) {
            background(new EmptyBackground(width, height));
            return this;
        }

        public CreateIceAgeRecipeCategory<T> build(String name, CreateIceAgeRecipeCategory.Factory<T> factory) {
            return build(CreateIceAge.asResource(name), factory);
        }
        ...
        public CreateIceAgeRecipeCategory<T> build(ResourceLocation id, CreateIceAgeRecipeCategory.Factory<T> factory) {
            ...
            CreateIceAgeRecipeCategory.Info<T> info = new CreateIceAgeRecipeCategory.Info<>(
        ...
        private void consumeAllRecipesOfType(Consumer<RecipeHolder<T>> consumer) {
            CreateIceAgeJEI.consumeAllRecipes(recipeHolder -> {
        ...
        private void consumeTypedRecipesTyped(Consumer<RecipeHolder<T>> consumer, net.minecraft.world.item.crafting.RecipeType<?> type) {
            CreateIceAgeJEI.consumeTypedRecipes(recipeHolder -> {
                if (recipeClass.isInstance(recipeHolder.value())) {
                    //noinspection unchecked - this is checked by the if statement
                    consumer.accept((RecipeHolder<T>) recipeHolder);
                }
