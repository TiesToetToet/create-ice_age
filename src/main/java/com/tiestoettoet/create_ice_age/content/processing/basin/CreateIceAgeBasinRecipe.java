...
public class CreateIceAgeBasinRecipe extends CreateIceAgeStandardProcessingRecipe<RecipeInput> {
    ...
    public static boolean match(BasinBlockEntity basin, Recipe<?> recipe) {
        ...
        boolean filterTest = filter.test(recipe.getResultItem(basin.getLevel()
                .registryAccess()));
        if (recipe instanceof CreateIceAgeBasinRecipe basinRecipe) {
    ...
    private static boolean apply(BasinBlockEntity basin, Recipe<?> recipe, boolean test) {
        boolean isBasinRecipeFreeze = recipe instanceof CreateIceAgeBasinRecipe;
        ...
        if (isBasinRecipeFreeze) {
            fluidIngredients = ((CreateIceAgeBasinRecipe) recipe).getFluidIngredients();
        } else if (isBasinRecipe) {
            fluidIngredients = ((BasinRecipe) recipe).getFluidIngredients();
        } else {
            fluidIngredients = Collections.emptyList();
        }
    ...
    public static RecipeHolder<CreateIceAgeBasinRecipe> convertShapeless(RecipeHolder<?> recipe) {
        CreateIceAgeBasinRecipe basinRecipe =
                new Builder<>(CreateIceAgeBasinRecipe::new, recipe.id()).withItemIngredients(recipe.value().getIngredients())
    ...
    protected CreateIceAgeBasinRecipe(IRecipeTypeInfo type, CreateIceAgeProcessingRecipeParams params) {
        super(type, params);
    }

    public CreateIceAgeBasinRecipe(CreateIceAgeProcessingRecipeParams params) {
