...
public class BreezeFreezerVisual extends AbstractBlockEntityVisual<BreezeFreezerBlockEntity> implements SimpleDynamicVisual, SimpleTickableVisual {
    ...
    public BreezeFreezerVisual(VisualizationContext ctx, BreezeFreezerBlockEntity blockEntity, float partialTick) {
        super(ctx, blockEntity, partialTick);

        freezingLevel = FreezingLevel.COOLING;
        validBlockAbove = blockEntity.isValidBlockAbove();

        PartialModel breezeModel = BreezeFreezerRenderer.getBlazeModel(freezingLevel, validBlockAbove);
        isInert = breezeModel == AllPartialModels.FREEZE_INERT;

        head = instancerProvider().instancer(InstanceTypes.TRANSFORMED, Models.partial(breezeModel))
                .createInstance();
    ...
    private void animate(float partialTicks) {
        ...
        if (validBlockAbove != this.validBlockAbove || freezingLevel != this.freezingLevel) {
            ...
            PartialModel breezeModel = BreezeFreezerRenderer.getBlazeModel(freezingLevel, validBlockAbove);
            instancerProvider().instancer(InstanceTypes.TRANSFORMED, Models.partial(breezeModel))
                    .stealInstance(head);
            ...
            if (needsRods && !hasRods) {
                PartialModel rodsModel = freezingLevel == FreezingLevel.SUPER_FREEZING ? AllPartialModels.BREEZE_FREEZER_SUPER_RODS
                        : AllPartialModels.BREEZE_FREEZER_RODS;
                PartialModel rodsModel2 = freezingLevel == FreezingLevel.SUPER_FREEZING ? AllPartialModels.BREEZE_FREEZER_SUPER_RODS_2
                        : AllPartialModels.BREEZE_FREEZER_RODS_2;
        ...
        if (blockEntity.goggles && goggles == null) {
            goggles = instancerProvider().instancer(InstanceTypes.TRANSFORMED, Models.partial(isInert ? AllPartialModels.BREEZE_GOGGLES_SMALL : AllPartialModels.BREEZE_GOGGLES))
                    .createInstance();
    ...
    private void setupFlameInstance() {
        flame = instancerProvider().instancer(AllInstanceTypes.SCROLLING, Models.partial(AllPartialModels.BREEZE_FREEZER_FLAME))
                .createInstance();
